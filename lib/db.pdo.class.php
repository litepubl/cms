...
class tdatabase extends PDO {
  ...
  public function __construct() {
    ...
    $this->history = array();
    try { {
        $init = $dbconfig['port'] > 0 ? "{$dbconfig['driver']}:host={$dbconfig['host']};port={$dbconfig['port']};dbname={$dbconfig['dbname']}" : "{$dbconfig['driver']}:host={$dbconfig['host']};dbname={$dbconfig['dbname']}";
      }
      parent::__construct($init, $dbconfig['login'], str_rot13(base64_decode($dbconfig['password'])) , array(
