...
doclose:function(){this.dialog&&b.isFunction(this.options.close)&&this.options.close(this.dialog);this.removeOnclose&&this.remove()},remove:function(){if(!this.dialog)return!1;this.footer=this.options=!1;this.dialog.find("tooltip-toggle").tooltip("destroy");this.dialog.remove();this.dialog=!1;this.style&&(this.style.remove(),this.style=!1)},addstyle:function(){var a=this.options,c=a.css;a.width&&(c=c+".modal-dialog{width:"+(a.width+this.padwidth)+"px}");a.height&&(c=c+".modal-dialog{height:"+(a.height+
this.padheight)+"px}");a.buttons.length||(c+=".modal-footer{display:none}");c&&(this.style=b('<style type="text/css">'+c+"</style>").appendTo("head:first"))},open:function(a){if(this.dialog)return alert("Dialog already opened");var c=litepubl.guid++;this.options=b.extend({},this.default_options,a);a=this.options.buttons;for(var e="",d=0,f=a.length;d<f;d++)var g=a[d].title,e=e+b.simpletml(this.button,{index:d,id:c,title:g.title,icon:"icon"in g?g.icon:""});1==a.length&&(e=e.replace(/btn-default/g,"btn-primary"));
c=b.simpletml(this.tml,{id:c,title:this.options.title,close:lang.dialog.close,body:this.options.html,buttons:e});this.addstyle();c=this.dialog=b(c).appendTo("body");this.footer=b(".modal-footer:first",c);d=0;for(f=a.length;d<f;d++)this.getbutton(d).on("click.dialog",a[d].click);return c.fixAndroid().on("shown.bs.modal.dialog",b.proxy(this.opened,this)).on("hidden.bs.modal.dialog",b.proxy(this.doclose,this)).modal()},opened:function(){b.isFunction(this.options.open)&&this.options.open(this.dialog)},
getbutton:function(a){return this.footer?this.footer.find("button[data-index="+a+"]"):!1},geticon:function(a){if(!a)return"";var b={info:"info-circle",success:"check",danger:"times-circle",error:"times-circle"};a in b&&(a=b[a]);return'<span class="fa fa-'+a+'"></span>'}});b.ready2(function(){"modal"in b.fn&&(b.bsdialog=new b.BootstrapDialog)})})(jQuery,window,document);
