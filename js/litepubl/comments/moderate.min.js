...
b.jsonrpc({type:"get",method:"comment_setstatus",params:{id:c,status:"hold"==a?"hold":"approved"},callback:function(g){try{if(0==g)return d.error(lang.comments.notmoderated);b("hold"==a?e.hold:e.comments).append(b(e.comment+c));d.setenabled(!0)}catch(f){erralert(f)}},error:function(a,b){d.error(lang.comments.notmoderated)}});break;case "edit":this.setenabled(!1);b.jsonrpc({type:"get",method:"comment_getraw",params:{id:c},callback:function(a){try{d.edit(c,a.rawcontent)}catch(b){erralert(b)}},error:function(a,
...
function(){if("confirmcomment"in litepubl)return litepubl.confirmcomment.submit()})},loadhold:function(){var c=this;b.jsonrpc({type:"get",method:"comments_get_hold",params:{idpost:ltoptions.idpost},callback:function(a){try{var d=ltoptions.theme.comments,e=d.holdcomments;if(d.ismoder&&e.length){for(;d.comments.next()[0]!=e[0];)d.comments.next().remove();e.remove()}d.comments.after(a.items);d.holdcomments=b(d.hold);c.create_buttons(d.holdcomments)}catch(f){erralert(f)}},error:function(a,b){c.error(lang.comments.errorrecieved)}})},
