function htmlentities(a){return $("<pre></pre>").text(a).html()}
var rpc={version:"0.8.0.2",requestCount:0,ServiceProxy:function(a,b){this.__serviceURL=a;this.__isCrossSite=!1;var d=this.__serviceURL.match(/^(\w+:)\/\/([^\/]+?)(?::(\d+))?(?:$|\/)/);d&&(this.__isCrossSite=location.protocol!=d[1]||document.domain!=d[2]||location.port!=(d[3]||""));var c;this.__isResponseSanitized=this.__isAsynchronous=!0;this.__authPassword=this.__authUsername=null;this.__callbackParamName="JSON-response-callback";this.__protocol="JSON-RPC";this.__dateEncoding="ISO8601";this.__decodeISO8601=
...
rpc.requestCount);var f=document.createElement("script");f.setAttribute("type","text/javascript");var g=this.__serviceURL+"/"+a+"?"+this.__callbackParamName+"=rpc.callbacks.r"+rpc.requestCount;b&&(g+="&"+rpc.toQueryString(b));f.setAttribute("src",g);f.setAttribute("id","rpc"+rpc.requestCount);var k=document.getElementsByTagName("head")[0];rpc.pendingRequests[rpc.requestCount].scriptElement=f;k.appendChild(f)}else{if(b&&(!(b instanceof Object)||b instanceof Date))throw Error("When making asynchronous calls, the parameters for the method must be passed as an array (or a hash); the value you supplied ("+
String(b)+') is of type "'+typeof b+'".');var l,p;if("XML-RPC"==this.__protocol){if(!(b instanceof Array))throw Error("Unable to pass associative arrays to XML-RPC services.");d=['<?xml version="1.0"?><methodCall><methodName>'+a+"</methodName>"];if(b){d.push("<params>");for(f=0;f<b.length;f++)d.push("<param>"+this.__toXMLRPC(b[f])+"</param>");d.push("</params>")}d.push("</methodCall>");p=d.join("")}else l={version:"1.1",method:a,id:rpc.requestCount},b&&(l.params=b),p=this.__toJSON(l);var h;if(window.XMLHttpRequest)h=
new XMLHttpRequest;else if(window.ActiveXObject)try{h=new ActiveXObject("Msxml2.XMLHTTP")}catch(t){h=new ActiveXObject("Microsoft.XMLHTTP")}h.open("POST",this.__serviceURL,this.__isAsynchronous,this.__authUsername,this.__authPassword);"XML-RPC"==this.__protocol?(h.setRequestHeader("Content-Type","text/xml"),h.setRequestHeader("Accept","text/xml")):(h.setRequestHeader("Content-Type","application/json"),h.setRequestHeader("Accept","application/json"));if(this.__isAsynchronous){h.send(p);var n=this,
q=rpc.requestCount;h.onreadystatechange=function(){if(4==h.readyState){if("XML-RPC"==n.__protocol)try{var a=n.__getXMLRPCResponse(h,q)}catch(b){alert("XML-RPC Error! "+b.message)}else a=n.__evalJSON(h.responseText,n.__isResponseSanitized),a.id||(a.id=q);n.__doCallback(a)}}}else{h.send(p);var m;m="XML-RPC"==this.__protocol?this.__getXMLRPCResponse(h,rpc.requestCount):this.__evalJSON(h.responseText,this.__isResponseSanitized);if(m.error)throw Error('Unable to call "'+a+'". Server responsed with error (code '+
m.error.code+"): "+m.error.message);this.__upgradeValuesFromJSON(m);return m.result}}}catch(r){if(a=!1,c&&(a=c(r)),e&&e(),!a)throw r;}};rpc.pendingRequests={};rpc.callbacks={};
...
a.id];b=[];if(void 0!==a.error){var d=Error(a.error.message);d.code=a.error.code;if(rpc.pendingRequests[a.id].onException)try{rpc.pendingRequests[a.id].onException(d)||b.push(d)}catch(c){b.push(d),b.push(c)}else b.push(d)}else if(void 0!==a.result&&(this.__upgradeValuesFromJSON(a),rpc.pendingRequests[a.id].onSuccess))try{rpc.pendingRequests[a.id].onSuccess(a.result)}catch(e){if(rpc.pendingRequests[a.id].onException)try{rpc.pendingRequests[a.id].onException(e)||b.push(e)}catch(f){b.push(e),b.push(f)}else b.push(e)}try{if(rpc.pendingRequests[a.id].onComplete)rpc.pendingRequests[a.id].onComplete(a)}catch(g){if(rpc.pendingRequests[a.id].onException)try{rpc.pendingRequests[a.id].onException(g)||
b.push(g)}catch(k){b.push(g),b.push(k)}else b.push(g)}delete rpc.pendingRequests[a.id];if(b.length){var l;a="There "+(1==b.length?"was 1 uncaught exception":"were "+b.length+" uncaught exceptions")+": ";for(d=0;d<b.length;d++)d&&(a+="; "),a+=b[d].message,b[d].code&&(l=b[d].code);d=Error(a);d.code=l;throw d;}};
...
rpc.ServiceProxy.prototype.__toXMLRPC=function(a){var b=["<value>"];switch(typeof a){case "number":isFinite(a)?parseInt(a)==Math.ceil(a)?(b.push("<int>"),b.push(a.toString()),b.push("</int>")):(b.push("<double>"),b.push(a.toString()),b.push("</double>")):b.push("<nil/>");break;case "boolean":b.push("<boolean>");b.push(a?"1":"0");b.push("</boolean>");break;case "string":b.push("<string>");b.push(htmlentities(a));b.push("</string>");break;case "object":if(null===a)b.push("<nil/>");else if(a instanceof
Array){b.push("<array><data>");for(var d=0;d<a.length;d++)b.push(this.__toXMLRPC(a[d]));b.push("</data></array>")}else if(a instanceof Date)b.push("<dateTime.iso8601>"+rpc.dateToISO8601(a)+"</dateTime.iso8601>");else{if(a instanceof Number||a instanceof String||a instanceof Boolean)return rpc.dateToISO8601(a.valueOf());b.push("<struct>");var d={}.hasOwnProperty?!0:!1,c;for(c in a)if(!d||a.hasOwnProperty(c))b.push("<member>"),b.push("<name>"+c+"</name>"),b.push(this.__toXMLRPC(a[c])),b.push("</member>");
b.push("</struct>")}break;default:throw new TypeError('Unable to convert the value of type "'+typeof a+'" to XML-RPC.');}b.push("</value>");return b.join("")};
...
if(isNaN(a))throw Error("XML-RPC Parse Error: The value provided as a double '"+b.firstChild.nodeValue+"' is invalid.");return a;case "boolean":if("0"!=b.firstChild.nodeValue&&"1"!=b.firstChild.nodeValue)throw Error("XML-RPC Parse Error: The value provided as a boolean '"+b.firstChild.nodeValue+"' is invalid.");return Boolean(parseInt(b.firstChild.nodeValue));case "string":return b.firstChild?b.firstChild.nodeValue:"";case "datetime.iso8601":a=new Date(0);if(b=b.firstChild.nodeValue.match(/^(?:(\d\d\d\d)-(\d\d)(?:-(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/))return b[1]&&
a.setUTCFullYear(parseInt(b[1])),b[2]&&a.setUTCMonth(parseInt(b[2]-1)),b[3]&&a.setUTCDate(parseInt(b[3])),b[4]&&a.setUTCHours(parseInt(b[4])),b[5]&&a.setUTCMinutes(parseInt(b[5])),b[6]&&a.setUTCMilliseconds(parseInt(b[6])),a;throw Error("XML-RPC Parse Error: The provided value does not match ISO8601.");case "base64":try{return b.firstChild.nodeValue.decode("base64")}catch(d){throw Error("Not able to parse base64 data yet.");}case "nil":return null;case "struct":for(var c={},e,f=0;e=b.childNodes.item(f);f++)if(1==
e.nodeType&&"member"==e.nodeName){var g="";a=null;for(var k,l=0;k=e.childNodes.item(l);l++)1==k.nodeType&&("name"==k.nodeName?g=k.firstChild.nodeValue:"value"==k.nodeName&&(a=k));g&&a&&(c[g]=this.__parseXMLRPC(a))}return c;case "array":c=[];for(b=b.firstChild;b&&(1!=b.nodeType||"data"!=b.nodeName);)b=b.nextSibling;b||Error("XML-RPC Parse Error: Expected 'data' element as sole child element of 'array'.");for(a=b.firstChild;a;){if(1==a.nodeType)if("value"==a.nodeName)c.push(this.__parseXMLRPC(a));else throw Error("XML-RPC Parse Error: Illegal element child '"+
a.nodeName+"' of an array's 'data' element.");a=a.nextSibling}return c;default:throw Error("XML-RPC Parse Error: Illegal element '"+b.nodeName+"' child of the 'value' element.");}return""};
